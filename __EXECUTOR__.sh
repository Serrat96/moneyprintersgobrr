...
DOCKER_BUILD="moneyprintersgobrr:latest"
...
sudo docker run -td -p 8501:8501 ${DOCKER_BUILD}
...
sudo docker exec ${DOCKER_ID} sh -c "nohup streamlit run ./src/dashboard/app_v4.py > app.log &"
